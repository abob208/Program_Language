...
enum VariableType { INT = 0, CHAR = 1, FLOAT = 2, STRING = 3, BOOL = 4, VOID = 5, NO = 6 };
struct Token {
  string name ;
  int column ;
  int line ;
...
struct Function {
  string name ;
  vector<Token> list ;
  vector<Variable> regionvariable ;
...
struct Reference {
  ...
  string ori_name ;
  int function_number ; // 他的address在的地方 
  bool isarray ;
  int arr_index ;
  int address ; 
  int scope ;
...
struct Regionlist {
  vector<Reference> reference_list ;
  vector<Variable> regionvariable ;
};
...
class Parser {
  ...
  bool IsDefined( string name, int now_scope ) ;   // 使用變數做計算 來判斷有沒有宣告過 
  bool Global_IsDefined( string name ) ; // 宣告變數時 判斷之前有沒有宣告過 
  ...
  vector<Variable> mRegionvariable ; // 區域變數 
  vector<Reference> mReferenceList ; // 記錄傳址的參數 
  vector<Regionlist> mStack ;
  int marr_index ; // 超偷懶 
  bool mOprator ;
  bool mYesorno ;
  bool mChange ;
  ...
  string mcout ;
  Value mfunctionvalue ; // function的值 
...
bool Parser::IsDefined( string name, int now_scope ) {
  
  for ( int i = 0 ; i < mVariableList.size() ; i++ ) {
    if ( mVariableList[i].name == name && mVariableList[i].scope <= now_scope ) {
      return true ;
    } // if
  } // for
  ...
  for ( int i = 0 ; i < mtempVariableList.size() ; i++ ) {
    if ( mtempVariableList[i].name == name && mtempVariableList[i].scope <= now_scope ) {
      return true ;
    } // if
  } // for
...
void Parser::Function_definition_without_ID() {
  ...
  if ( mnexttoken.name == "(" ) {
    ...
    if ( !Global_IsDefined( mtokenList[1].name ) ) {
      ...
      function.parameters = mtempFunctionParameter ;
      mFunctionList.push_back( function ) ;
    } // if
    else {
...
void Parser::Basic_expression() {
  Scanner scanner ;
  if ( mnexttoken.type == IDENT ) {
    if ( !IsDefined( mnexttoken.name, gscope ) ) 
...
void Parser::Set_Variable_value( Value value, string ID ) {
  ...
  int long_temp2 = 0 ;
  int long_temp3 = 0 ;
  for ( int i = 0 ; mfunction != -1 && i < mStack[mfunction].reference_list.size() && gscope > 0 ; i++ ) {
    if ( ID == mStack[mfunction].reference_list[i].function_name ) {
      if ( mStack[mfunction].reference_list[i].function_number != -1 ) {

        if ( mStack[mfunction].reference_list[i].isarray ) {
          long_temp = mStack[mfunction].reference_list[i].function_number ;
          long_temp2 = mStack[mfunction].reference_list[i].address ;
          long_temp3 = mStack[mfunction].reference_list[i].arr_index ;
          if ( value.type == 0 || value.type == 2 ) {
            mStack[long_temp].regionvariable[long_temp2].value.num_array[long_temp3] = value.num_value ;
            mStack[long_temp].regionvariable[long_temp2].value.num_value = value.num_value ;
          } // if
          else if ( value.type == 1 ) {
            mStack[long_temp].regionvariable[long_temp2].value.char_array[long_temp3] = value.char_value ;
            mStack[long_temp].regionvariable[long_temp2].value.char_value = value.char_value ;
          } // else if
          else if ( value.type == 3 ) {
            mStack[long_temp].regionvariable[long_temp2].value.str_array[long_temp3] = value.str_value ;
            mStack[long_temp].regionvariable[long_temp2].value.str_value = value.str_value ;
          } // else if
        } // if
        else {
          long_temp = mStack[mfunction].reference_list[i].function_number ;
          mStack[long_temp].regionvariable[mStack[mfunction].reference_list[i].address].value = value ;
        } // else
      } // if
      else {
        if ( mStack[mfunction].reference_list[i].isarray ) {
          ...
          long_temp3 = mStack[mfunction].reference_list[i].arr_index ;
          if ( value.type == 0 || value.type == 2 ) {
            mVariableList[long_temp2].value.num_array[long_temp3] = value.num_value ;
            mVariableList[long_temp2].value.num_value = value.num_value ;
          } // if
          else if ( value.type == 1 ) {
            mVariableList[long_temp2].value.char_array[long_temp3] = value.char_value ;
            mVariableList[long_temp2].value.char_value = value.char_value ;
          } // else if
          else if ( value.type == 3 ) {
            mVariableList[long_temp2].value.str_array[long_temp3] = value.str_value ;
            mVariableList[long_temp2].value.str_value = value.str_value ;
          } // else if
        } // if
        else
          mVariableList[mStack[mfunction].reference_list[i].address].value = value ;
      } // else
  ...
  for ( int i = 0 ; mfunction != -1 && i < mStack[mfunction].regionvariable.size() ; i++ ) {
    if ( ID == mStack[mfunction].regionvariable[i].name ) {
      mStack[mfunction].regionvariable[i].value = value ;
      return ;
    } // if
  } // for
...
Value Parser::IDvalue( string ID ) {
  string temp_cout = mcout ;
  int long_temp = 0 ;
  Variable temp ;
  vector<Token> temp_token_list ;
  temp_token_list = mtokenList ; // 這可能要放在function struct裡面 
  int temp_index = mindex ; // 跟上面依樣 
  Value value ;
  ...
  for ( int i = 0 ; mfunction != -1 && i < mStack[mfunction].reference_list.size() && gscope > 0 ; i++ ) {
    ...
    if ( ID == mStack[mfunction].reference_list[i].function_name ) {
      if ( mStack[mfunction].reference_list[i].function_number != -1 ) {
        ...
        long_temp = mStack[mfunction].reference_list[i].function_number ;
        return mStack[long_temp].regionvariable[mStack[mfunction].reference_list[i].address].value ;
      } // if
      else {

        return mVariableList[mStack[mfunction].reference_list[i].address].value ;
      } // else
    } // if
  } // for
  ...
  for ( int i = 0 ; mfunction != -1 && i < mStack[mfunction].regionvariable.size() ; i++ ) {
    if ( ID == mStack[mfunction].regionvariable[i].name )
      return mStack[mfunction].regionvariable[i].value ;
  } // for
  ...
  for ( int i = 0 ; i < mFunctionList.size() ; i++ ) {
    if ( ID == mFunctionList[i].name ) {
      mtokenList = mFunctionList[i].list ;
      mindex = 0 ;
...
void Parser::CAL_Rest_of_declarators() {
  Variable variable ;
  variable.isarray = false ;
  variable.isfloat = false ;
  variable.arraysize = 0 ;
  variable.type = Whattype( mtokenList[mindex - 2].name ) ;
  variable.value.type = Whattype( mtokenList[mindex - 2].name ) ;
  variable.name = mtokenList[mindex - 1].name ;
  variable.scope = gscope ; 
  ...
  mStack[mfunction].regionvariable.push_back( variable ) ;
  variable.isarray = false ;
  variable.isfloat = false ;
  variable.arraysize = 0 ;
  variable.scope = gscope ;
  while ( mtokenList[mindex].name == "," ) {
    ...
    if ( mtokenList[mindex].type == IDENT ) {
      variable.name = mtokenList[mindex].name ;
      mindex++ ;
    } // if
...
void Parser::CAL_Statement() {
  ...
  if ( mtokenList[mindex].name == ";" ) {
    ...
    if ( mtokenList[mindex].type == IDENT || mtokenList[mindex].name == "++" || 
      ...
      value.type = Whattype( mtokenList[0].name ) ;
      mfunctionvalue = value ;
      // cout << mfunctionvalue.num_value ;
    } // if 
    ...
    while ( mindex < mtokenList.size() - 1 )
      mindex++ ;
    // cout << mfunctionvalue.num_value << "---------------------------------------------";
  } // else if
    ...
    if ( mtokenList[mindex].name == "(" ) {
      ...
      if ( mtokenList[mindex].name == "else" && do_else ) {
        mindex++ ;
        CAL_Statement() ;
      } // if
      else if ( mtokenList[mindex].name == "else" ) {
        mindex++ ;
        Skip_Statement() ;
      } // else if
    } // if
...
void Parser::CAL_Basic_expression( Value & value ) {
  ...
  if ( mtokenList[mindex].type == IDENT ) {
    ...
    if ( mtokenList[mindex].type == IDENT ) {
      ...
      mindex++ ;
      CAL_Rest_of_PPMM_Identifier_started_basic_exp( value, mtokenList[mindex - 2].name ) ;
    } // if
  } // else if
  else if ( mtokenList[mindex].name == "+" || mtokenList[mindex].name == "-"  || 
...
void Parser::CAL_Actual_parameter_list( Value & value ) {
  ...
  string temp = "" ;
  temp =  mtokenList[mindex].name ;
  tempID = mtokenList[mindex - 2].name ;
  CAL_Basic_expression( value ) ;
  for ( i = 0 ; i < mFunctionList.size() ; i++ ) {
    if ( mFunctionList[i].name == tempID ) {

      if ( mFunctionList[i].parameters[parameters_index].isarray || 
           mFunctionList[i].parameters[parameters_index].reference ) {
        // cout << "@@@@" ;



        reference.function_name = mFunctionList[i].parameters[parameters_index].name ;
        reference.ori_name = temp ;
        reference.scope = gscope ;
        for ( int k = 0 ; mfunction != -1 && k < mStack[mfunction].reference_list.size() ; k++ ) {
          if ( reference.ori_name == mStack[mfunction].reference_list[k].function_name ) {
            reference.function_number = mStack[mfunction].reference_list[k].function_number ;
            reference.address = mStack[mfunction].reference_list[k].address ;
            reference.isarray = mStack[mfunction].reference_list[k].isarray ;
            reference.arr_index = mStack[mfunction].reference_list[k].arr_index ;

            indo = true ;
          } // if
        } // for

        for ( int k = 0 ; mfunction != -1 && k < mStack[mfunction].regionvariable.size() && !indo ; k++ ) {
          if ( reference.ori_name == mStack[mfunction].regionvariable[k].name ) {
            if ( mStack[mfunction].regionvariable[k].isarray 
                 && mFunctionList[i].parameters[parameters_index].reference 
                 && !mFunctionList[i].parameters[parameters_index].isarray ) {
              reference.isarray = true ;
              reference.arr_index = marr_index ;
              temp4 = mStack[mfunction].regionvariable[k].value.num_array[marr_index] ;
              temp5 = mStack[mfunction].regionvariable[k].value.char_array[marr_index] ;
              temp6 = mStack[mfunction].regionvariable[k].value.str_array[marr_index] ;
              if ( mStack[mfunction].regionvariable[k].value.type == 0 
                   || mStack[mfunction].regionvariable[k].value.type == 2 )
                mStack[mfunction].regionvariable[k].value.num_value = temp4 ;
              else if ( mVariableList[k].value.type == 1 )
                mStack[mfunction].regionvariable[k].value.char_value = temp5 ;
              else if ( mVariableList[k].value.type == 3 )
                mStack[mfunction].regionvariable[k].value.str_value = temp6 ;
            } // if

            reference.address = k ;
            reference.function_number = mfunction ;
            indo = true ;
          } // if
        } // for
        
        for ( int k = 0 ; k < mVariableList.size() && !indo ; k++ ) {
          if ( reference.ori_name == mVariableList[k].name ) {
            if ( mVariableList[k].isarray && mFunctionList[i].parameters[parameters_index].reference 
                 && !mFunctionList[i].parameters[parameters_index].isarray ) {
              reference.isarray = true ;
              reference.arr_index = marr_index ;
              if ( mVariableList[k].value.type == 0 || mVariableList[k].value.type == 2 )
                mVariableList[k].value.num_value = mVariableList[k].value.num_array[marr_index] ;
              else if ( mVariableList[k].value.type == 1 )
                mVariableList[k].value.char_value = mVariableList[k].value.char_array[marr_index] ;
              else if ( mVariableList[k].value.type == 3 )
                mVariableList[k].value.str_value = mVariableList[k].value.str_array[marr_index] ;
            } // if

            reference.address = k ;
            reference.function_number = -1 ;
            indo = true ;
          } // if
        } // for


        // mReferenceList.push_back( reference ) ;

        mStack[mfunction + 1].reference_list.push_back( reference ) ;

      } // if
      else {
        mFunctionList[i].parameters[parameters_index].value = value ;
        mFunctionList[i].parameters[parameters_index].type = value.type ;
        mFunctionList[i].parameters[parameters_index].scope = gscope + 1 ;
        mStack[mfunction + 1].regionvariable.push_back( mFunctionList[i].parameters[parameters_index] ) ;
  ...
  while ( mtokenList[mindex].name == "," ) {
    ...
    temp = mtokenList[mindex].name ;
    CAL_Basic_expression( value ) ;
    if ( mFunctionList[j].parameters[parameters_index].isarray ||
         mFunctionList[j].parameters[parameters_index].reference ) {
      reference.function_name = mFunctionList[j].parameters[parameters_index].name ;
      reference.ori_name = temp ;
      reference.scope = gscope ;
      // mReferenceList.push_back( reference ) ;
      for ( int k = 0 ; mfunction != -1 && k < mStack[mfunction].reference_list.size() ; k++ ) {
      ...
      for ( int k = 0 ; mfunction != -1 && k < mStack[mfunction].regionvariable.size() && !indo ; k++ ) {
        if ( reference.ori_name == mStack[mfunction].regionvariable[k].name ) {
          if ( mStack[mfunction].regionvariable[k].isarray 
               && mFunctionList[j].parameters[parameters_index].reference 
               && !mFunctionList[j].parameters[parameters_index].isarray ) {
            reference.isarray = true ;
            reference.arr_index = marr_index ;
            temp4 = mStack[mfunction].regionvariable[k].value.num_array[marr_index] ;
            temp5 = mStack[mfunction].regionvariable[k].value.char_array[marr_index] ;
      ...
      mFunctionList[j].parameters[parameters_index].value = value ;
      mFunctionList[j].parameters[parameters_index].type = value.type ;
      mFunctionList[j].parameters[parameters_index].scope = gscope + 1 ;

      mStack[mfunction + 1].regionvariable.push_back( mFunctionList[j].parameters[parameters_index] ) ;
...
void Parser::CAL_Maybe_shift_exp( Value & value ) {
  ...
  while ( mtokenList[mindex].name == "<<" || mtokenList[mindex].name == ">>" ) {
    mindex++ ;
    CAL_Maybe_additive_exp( value ) ;
    if ( mcout == "cout" && value.type == STRING && misprint ) {
    ...
    else if ( mcout == "cout" && value.type == INT && misprint ) {
      num2 = value.num_value ;
      cout << num2 ;
    } // else if
    else if ( mcout == "cout" && value.type == FLOAT && misprint ) {
      num = value.num_value ;
      cout << num ;
    } // else if
    else if ( mcout == "cout" && value.type == CHAR && misprint )
      cout << value.char_value ;
    else if ( mcout == "cout" && value.type == BOOL && misprint ) {
      if ( value.bool_value )
        cout << "true" ;
